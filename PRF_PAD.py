...
class PRF_PAD(object):
        ...
        def Process_ASCII_File(self):
                ...
                self.XVx = np.array(XV)
                self.YVx = np.array(YV)
...
##                self.ModArr = self.interpNaN(self.ModArr*-1)
                ...
                if len(np.array(self.XV)[mask]) < 307200:
                    ZV = PRF_Algo().interpNaN(np.array(self.XV)[mask], np.array(self.YV)[mask], np.array(ZV)[mask], self.XVx, self.YVx)
...
##                print "Via Depth: " + str(AvgTopHt - AvgBotHt)
        ...
        def getFigObj(self, bgColor, saveFig=False):
                ...
                f2_ax1.plot (self.topCircle[6], self.topCircle[7], 'r-', linewidth=2.0, label="Top Circle Fit") #Circle Fit plot
                f2_ax1.plot (self.botCircle[6], self.botCircle[7], 'b-', linewidth=2.0, label="Bot Circle Fit") #Circle Fit plot
                contourf_ = f2_ax1.contourf(self.X_Array, self.Y_Array, self.DataArr , 60, cmap=plt.get_cmap('jet'))#, cmap=plt.cm.mpl)
...
##                f2_ax1.text(txtPos, int(160*self.Meas_Pix), 'Rsk:        ' + str(round(self.Rsk,4)),fontsize=12)
                ...
                #X profile Creation
                f2_ax3.plot(self.X_Array, self.DataArr[int(float(self.crossX)/self.Meas_Pix)], 'b')
                ...
                #Y profile Creation
                f2_ax4.plot(self.Y_Array, self.DataArr[:,int(float(self.crossY)/self.Meas_Pix)], 'r')
                ...
                counts, bins = np.histogram(self.DataArr[mask], bins=200)
        ...
        def getUpdatedProfile(self, xVal, yVal):
                self.crossX = xVal
                self.crossY = yVal
                return [self.X_Array, self.DataArr[int(float(xVal)/self.Meas_Pix)], self.Y_Array, self.DataArr[:,int(float(yVal)/self.Meas_Pix)]]
